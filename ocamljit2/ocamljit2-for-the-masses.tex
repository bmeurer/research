...
includes the new x86 port, a simple native code managment strategy, as well as improvements
to the implementation of floating-point primitives, which caused another speedup of around
$30\%$ for some floating-point benchmarks.
...
\subsection{Native code managment}
...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}
    ...
    \draw (4.5,6.2) node {\footnotesize $\mbox{chunk pool}$};
...
segment is using \texttt{Meta.static\_release\_bytecode}, all associated native code
chunks are also released to the chunk pool. This way \textsc{OCamlJit2} no longer leaks
