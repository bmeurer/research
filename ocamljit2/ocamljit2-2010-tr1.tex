...
The offsets are therefore values in the range $-2^{27}+x \ldots{-1}$.
...
Now whenever a jump to a not yet compiled byte-code instruction is performed, the byte-code trampoline
...
and callee-save registers \cite{Matz10}. Now the compile trampoline preserves \texttt{\%rax} in a pre-allocated
cell on the native machine stack and invokes \texttt{caml\_jit\_compile} with the byte-code address to
compile in \texttt{\%rdi}. Once \texttt{caml\_jit\_compile} returns, \texttt{\%rax} contains the
address of the generated native code, so we save that address to \texttt{\%rdx}, restore the previous
