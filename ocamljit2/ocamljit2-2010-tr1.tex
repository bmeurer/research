...
64-bit Mac OS X 10.6, but should also run on \linebreak[4]Linux/amd64 and FreeBSD/amd64
systems without modifications.

The paper is organized as follows: Section~\ref{section:Existing_systems} presents the
existing systems (the relevant parts of the \textsc{OCaml} implementation, and the
...
Section~\ref{section:Related_work} and \ref{section:Further_work} list related and further
...
the values of the free variables, thereby forming the environment of the closure\footnote{Mutually
recursive closures -- used to implement the \texttt{let rec} language construct -- may contain
more than just one byte-code address.}.
...
\begin{itemize}
\item Stack manipulation instructions: $\mathtt{ACC}_p$ loads \texttt{accu} with the value $p$-th top-most stack
...
\item Apply operations are divided into two categories: $\mathtt{APPLY}_p$ creates a new call frame on
  ...
  the new \texttt{env}. Call frames contain arguments, byte-code return address and previous
...
\item The limited register set provided by \textsc{Gnu Lightning} prevents efficient register
  usage for modern targets like x86-64\footnote{Also known as \emph{AMD64}, but
  we prefer the vendor-neutral term.}, ARM or PowerPC, which offer more than just 8 general
...
Care must taken on 64-bit targets, as the addressable range is limited to $-2^{31} \ldots 2^{31}-1$ bytes,
...
stack-based languages like \textsc{Forth} \cite{Ertl93}. Several of the
ideas used in compiling \textsc{Forth} to efficient native machine code
\cite{Ertl92,Ertl96,ErtlPirker97,ErtlPirker98} may also be applicable in
\textsc{OCaml} byte-code to native code compilation.
...
We would like to thank Christian Uhrhan for his careful proof-reading.
