...
POSIX compliant operating system supported by OCaml. It is verified to work with Mac OS X 10.6 and
10.7, Debian GNU Linux 6.0 and above, and CentOS 5.6 and 5.7. The full source code
is available from the \texttt{ocamljit-nat} branch of the \texttt{ocaml-experimental} Git
repository hosted on GitHub at \cite{Meurer11ocamlexperimental}.
...
compiles OCaml programs to Xen micro-kernels, which are then executed by the Xen hypervisor; Mirage uses
...
\begin{figure*}[bt]
  ...
  \begin{tikzpicture}
    ...
    \end{scope}
    \begin{scope}[xshift=0cm,yshift=-4.4cm] % Intel Xeon E5520, CentOS 5.6 (amd64)
      \foreach \y in {1,...,4}  %Hilfslinien
      {
      ...
      \draw (-0.1cm,0cm) -- (-0.2cm,0cm);  %unteres Ende der Ordinate
      \draw (-0.1cm,2.2cm) -- (-0.2cm,2.2cm);  %oberes Ende der Ordinate
      \node at (3.3cm, 2.5cm) {\small Intel Xeon E5520, CentOS 5.6 (amd64)};
      \foreach \name/\x/\a/\b/\c in
      {
...
benchmarks, and up to hundred times faster in one benchmark), at moderate maintenance costs. It also
beats our earlier byte code based OCamlJIT2 prototype in almost every case (except for the
...
$O(n^2)$ case of the graph coloring algorithm. This work is also part of the first author's diploma
thesis.
