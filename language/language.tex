...
% \begin{definition}
%   Seien $A,B$ beliebige Mengen, $\L$ eine Syntax und $\Obs$ eine Menge von beoachtbaren Verhalten.
%   Eine \emph{$(A,B,\L,\Obs)$-Implementierung} ist ein Tupel $M = (\Exp,A,B,\semantic{\cdot},\obs^B)$ mit
%   \begin{enumerate}
%   \item $\Exp \supseteq \L$, 
%   \item $\semantic{\cdot}:\Exp\to(A \to B)$ und
%   \item $\obs^B:B \to \powerset{\Obs}$.
%   \end{enumerate}
% \end{definition}

% Offensichtlich ist jede $(A,B,\L,\Obs)$-Implementierung $M=(\Exp,A,B,\semantic{\cdot},\obs^B)$
% auch eine $(\L,\Obs)$-Implementierung $(\Exp,D,\semantic{\cdot},\obs)$ mit
% \begin{enumerate}
% \item $D = (A \to B)$ und
% \item $\obs: D \to \powerset{\Obs}, d \mapsto \bigcup_{a \in A} \obs^B(d\,a)$.
% \end{enumerate}
