...
\begin{proof}
  ...
  \end{array}\]
%  \[\xymatrix{
%    \semantic{\theta}_L \ar@/^1pc/[r]^{\mu^\theta} &
%    \semantic{\theta}_{\mu(L)} \ar@/^1pc/[l]^{(\mu^{-1})^\theta}
%  }\]
...
for the appropriate rules). Define $\supp{t} = \locns{t}$ for the current soundness lemma below.
...
\begin{lemma}
  Let $\sigma \in \SType$, $t \in \Term^\sigma$, $\eta,\hat{\eta} \in \Env_t$. Then
  \begin{enumerate}
    \item $(\semantic{\tj{t}{\sigma}}\,\eta) \in \semantic{\sigma}_{\supp{t, \eta|_{\free{t}}}}$
    \item $(\eta|_{\free{t}}) = (\hat{\eta}|_{\free{t}})$ implies
          $(\semantic{\tj{t}{\sigma}}\,\eta) = (\semantic{\tj{t}{\sigma}}\,\hat{\eta})$
    \item $\mu^\sigma\,(\semantic{\tj{t}{\sigma}}\,\eta) = \semantic{\tj{t}{\sigma}}\,(\mu^\senv\,\eta)$
          for all $\mu\in\Fix{\supp{t}}$
  \end{enumerate}
\end{lemma}
...
\begin{proof}
  ...
  Let $L = \supp{\tabstr{x}{t},\eta|_{\free{\tabstr{x}{t}}}}
         = \supp{t,\eta|_{\free{t} \setminus \{x\}}}$. In order to prove
  ...
  \begin{enumerate}
    ...
    \item[(b)] $\psi = \mu^{\sigma'} \circ \psi \circ (\mu^\sigma)^{-1}$ for every $\mu\in\Fix{L}$.
  \end{enumerate}
  {\em Proof of (a):} Let $d \in \semantic{\sigma}$. By induction hypothesis we have
  \[\begin{array}{rcl}
    \psi\,d = (\semantic{\tj{t}{\sigma'}}\,(\eta[d/x]))
    &\in& \semantic{\sigma'}_{\supp{t,(\eta[d/x])|_{\free{t}}}}
  \end{array}\]
  and we conclude
  \[\begin{array}{rcl}
    \supp{t,(\eta[d/x])|_{\free{t}}}
    &=& \supp{t,\eta|_{\free{t}\setminus\{x\}},d} \\
    &=& L \cup \supp{d}.
  \end{array}\]
   ...
   \[\begin{array}{rcl}
    ...
    &=& \semantic{\tj{t}{\sigma'}}\,((\mu^\senv\,\eta)[\mu^\sigma\,d/x]) \\
    &=& \semantic{\tj{t}{\sigma'}}\,(\mu^\senv\,(\eta[d/x])) \\
    &=& \mu^{\sigma'}\,(\semantic{\tj{t}{\sigma'}}\,(\eta[d/x])).
  \end{array}\]
  Hence we know that $\psi \in \semantic{\ttarrow{\sigma}{\sigma'}}_L$. We still need to show
  that {\bf TODO}
\end{proof}
