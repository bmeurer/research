...
\begin{itemize}
  ...
  \item the set $\LType$ of all {\em logical types} $\kappa$
\end{itemize}
...
\begin{definition}[Semantic domains]
  For every type $\kappa\in\LType$ we define {\em semantic domains} $\semantic{\kappa} = (D^\kappa,\I^\kappa)$ where
  $D^\kappa = \bigcup_{L \in W} D^\kappa_L$ and $\I^\kappa(\mu) : D^\kappa \to D^\kappa$ for every $\mu \in \Perm$ by
  \begin{itemize}
    ...
    \item $D^{\ttarrow{\sigma}{\kappa}}_L = \{ \psi: D^\sigma \to D^\kappa\,|\,
                          \forall L'\in W.\,\psi\,(D^\sigma_{L'}) \subseteq D^\kappa_{L \cup L'} \\
                          \hspace*{4.2cm} \wedge \forall \mu\in\Fix{L}.\,\psi=\mu^\kappa\circ\psi\circ(\mu^\sigma)^{-1}\}$\\
          $\I^{\ttarrow{\sigma}{\kappa}}(\mu): D^{\ttarrow{\sigma}{\kappa}} \to D^{\ttarrow{\sigma}{\kappa}},
                                            \psi \mapsto \I^\kappa(\mu) \circ \psi \circ (\I^\sigma(\mu))^{-1}$
  \end{itemize}
\end{definition}

We follow the usual mathematical convention and use $\semantic{\kappa}$ not only as a notation
for the semantic domain $(D^\kappa,\I^\kappa)$ but also for the underlying set $D^\kappa$,
hence $\semantic{\kappa}_L$ denotes the set $D^\kappa_L$. Moreover, we use $\mu^\kappa$ as an abbreviation
for $\I^\kappa(\mu)$.

Given lemma~\ref{lemma:Permutations}
it is pretty much obvious that $\mu^\kappa\,d = d$ for every $d \in \semantic{\kappa}_L$ and
$\mu\in\Fix{L}$.
It is also easy to see that
\[\begin{array}{rcl}
  \semantic{\kappa}_{L_1} \cap \semantic{\kappa}_{L_2} \ne \emptyset &\Rightarrow& L_1=L_2
\end{array}\]
holds for all $L_1,L_2\in W$. Thereby for each $d \in \semantic{\kappa}$ a
unique world $L \in W$ exists, such that $d \in \semantic{\kappa}_L$ and $d \not\in \bigcup_{L'\ne L} \semantic{\kappa}_{L'}$.
...
\begin{lemma}[Semantic domains] \
  Let $\kappa \in \LType$, $\mu\in\Perm$ and $L\in W$.
  \begin{enumerate}
    \item $(\mu^\kappa|\semantic{\kappa}_L): \semantic{\kappa}_L \to \semantic{\kappa}_{\mu(L)}$ is
          well-defined and bijective.
    \item $\mu^\kappa: \semantic{\kappa} \to \semantic{\kappa}$ is well-defined and bijective.
  \end{enumerate}
\end{lemma}

\begin{proof}
  Given that $\semantic{\kappa}$ is the union of disjoint sets $\semantic{\kappa}_L$, the second
  property is simply an implication of the first one. Now assume that $(\mu^\kappa|\semantic{\kappa}_L)$
  is well-defined, i.e. $\mu^\kappa\,\semantic{\kappa}_L \subseteq \semantic{\kappa}_{\mu(L)}$, then
  we also have $(\mu^{-1})^\kappa\,\semantic{\kappa}_{\mu(L)} \subseteq \semantic{\kappa}_L$, which
  implies
  \[\begin{array}{rcl}
    (\mu^\kappa \circ (\mu^{-1})^\kappa)\,\semantic{\kappa}_{\mu(L)} &\subseteq& \semantic{\kappa}_{\mu(L)}
  \end{array}\]
  and
  \[\begin{array}{rcl}
    ((\mu^{-1})^\kappa \circ \mu^\kappa)\,\semantic{\kappa}_L &\subseteq& \semantic{\kappa}_L.
  \end{array}\]
%  \[\xymatrix{
%    \semantic{\kappa}_L \ar@/^1pc/[r]^{\mu^\kappa} &
%    \semantic{\kappa}_{\mu(L)} \ar@/^1pc/[l]^{(\mu^{-1})^\kappa}
%  }\]
  Given these results it is then sufficient to prove that $(\mu^\kappa|\semantic{\kappa}_L)$ is
  well-defined and $\mu^\kappa \circ (\mu^{-1})^\kappa$ is the identity on $\semantic{\kappa}$.
  \begin{itemize}
    ...
    \item For $\mu^{\ttarrow{\sigma}{\kappa}}$ the statement follows immediately using the induction hypothesis.
%    \item Let $\psi\in\semantic{\ttarrow{\sigma}{\kappa}}_L$, then
%          \begin{itemize}
%            \item $d \in \semantic{\sigma}_{L'}, L'\in W \\
%                   \,\Rightarrow\,
%                   ((\mu^\sigma)^{-1}\,d) \in \semantic{\sigma}_{\mu^{-1}(L')} \\
%                   \,\Rightarrow\,
%                   \psi\,((\mu^\sigma)^{-1}\,d) \in \semantic{\kappa}_{L\cup\mu^{-1}(L')} \\
%                   \,\Rightarrow\,
%                   \mu^\kappa\,(\psi\,((\mu^\sigma)^{-1}\,d))\in\semantic{\kappa}_{\mu(L)\cup L'} \\
%                   \,\Rightarrow\,
%                   ((\mu^\kappa\circ\psi\circ(\mu^\sigma)^{-1})\,d)\in\semantic{\kappa}_{\mu(L)\cup L'}$
%
%            \item $\bar{\mu}\in\Perm_{\mu(L)} \\
%                   \,\Rightarrow\,
%                   (\mu^{-1}\circ \bar{\mu} \circ \mu) \in \Fix{L} \\
%                   \,\Rightarrow\,
%                   \psi = (\mu^{-1}\circ\bar{\mu}\circ\mu)^\kappa\circ\psi\circ
%                      ((\mu^{-1}\circ\bar{\mu}\circ\mu)^\sigma)^{-1} \\
%                   \,\Rightarrow\,
%                   \psi = (\mu^\kappa)^{-1}\circ\bar{\mu}^\kappa\circ\mu^\kappa\circ\psi
%                      \circ(\mu^\sigma)^{-1}\circ(\bar{\mu}^\sigma)^{-1}\circ\mu^\sigma \\
%                   \,\Rightarrow\,
%                   \mu^\kappa \circ \psi \circ (\mu^\sigma)^{-1}
%                      = \bar{\mu}^\kappa\circ[\mu^\kappa\circ\psi\circ(\mu^\sigma)^{-1}]\circ(\bar{\mu}^\sigma)^{-1}$
%          \end{itemize}
%          so we conclude $(\mu^{\ttarrow{\sigma}{\kappa}}\,\psi)\in\semantic{\ttarrow{\sigma}{\kappa}}_{\mu(L)}$.
%
%          Given $\mu^\sigma\circ(\mu^{-1})^\sigma = \id_{\semantic{\sigma}}$ and
%          $\mu^\kappa\circ(\mu^{-1})^\kappa = \id_{\semantic{\kappa}}$ by induction hypothesis it follows easily that
%          \[\begin{array}{rcccl}
%            (\mu^{\ttarrow{\sigma}{\kappa}}\circ (\mu^{-1})^{\ttarrow{\sigma}{\kappa}})\,\psi
%            &=& \mu^\kappa \circ (\mu^{-1})^\kappa \circ \psi \circ (\mu^{-1})^\sigma \circ \mu^\sigma 
%            &=& \psi.
%          \end{array}\]
  \end{itemize}
\end{proof}
...
\begin{lemma} \label{lemma:Permutations_and_semantic_domains}
  Let $\mu,\hat{\mu}\in\Perm$, $\kappa\in\LType$ and $d \in \semantic{\kappa}$, then
  $\mu^\kappa\,d = \hat{\mu}^\kappa\,d$ if
...
\begin{definition}[Environment] \label{definition:Environment}
  An {\em environment} is a partial function $\eta: \Var \pto \bigcup_{\kappa\in\LType} \semantic{\kappa}$ with
  a finite domain and $\eta(\Var^\kappa) \subseteq \semantic{\kappa}$ for every $\kappa \in \LType$. $\Env$ denotes
...
For every term $t \in \Term$, $\sigma \in \SType$ with $\tj{t}{\sigma}$ the {\em meaning function}
$\semantic{\tj{t}{\sigma}}: \Env_t \to \semantic{\sigma}$ is defined by
\begin{itemize}
  \item $\semantic{\tj{v}{\tau}}\,\eta := v\,\eta$

  \item $\semantic{\tj{f}{\sigma}}\,\eta := f^\I$

  \item $\semantic{\tj{\app{t_1}{t_2}}{\sigma}}\,\eta :=
          (\semantic{\tj{t_1}{\ttarrow{\sigma'}{\sigma}}}\,\eta)\,
          (\semantic{\tj{t_2}{\sigma'}}\,\eta)$

  \item $\semantic{\tj{\tabstr{x^\sigma}{t}}{\ttarrow{\sigma}{\sigma'}}}\,\eta := \psi$ \\
...
\begin{itemize}
  ...
  \item $\I_{\not\rightleftharpoons}: \semantic{\kappa_1} \to \semantic{\kappa_2} \to \semantic{\tassn}$ \\[1ex]
  ...
  \item $\I_\exists: \semantic{\kappa_1} \to (\semantic{\kappa_2} \to \semantic{\tassn}) \to \semantic{\tassn}$ \\[1ex]
        $\I_\exists\,d_1\,\psi\,s =
          \bcase
            \uparrow  & \text{if } s \not\in \dom{\psi\,d_2}
                        \text{ for some }d_2 \in \bigcup_{L'\subseteq\reach{\supp{d_1}}{s}}\semantic{\kappa}_{L'} \\
...
\begin{itemize}
  ...
  \item $\semantic{\tj{\tabstr{x^\sigma}{p}}{\ttarrow{\sigma}{\pi}}}\,\eta := \psi$ \\[1ex]
  ...
  \item $\semantic{\tj{t \not\rightleftharpoons p}{\tassn}}\,\eta :=
          \I_{\not\rightleftharpoons}\,(\semantic{\tj{t}{\sigma}}\,\eta)\,(\semantic{\tj{p}{\pi}}\,\eta)$

  \item $\semantic{\tj{\qbex{x^\kappa}{t}{p}}{\tassn}}\,\eta :=
          \I_\exists\,(\semantic{\tj{t}{\sigma}}\,\eta)\,\psi$ \\[1ex]
        $\psi: \semantic{\kappa} \to \semantic{\tassn}, d \mapsto \semantic{\tj{p}{\tassn}}\,(\eta[d/x])$
...
\begin{itemize}
  ...
  \item $\eta \models \qex{x^\kappa}{h}$ iff $\eta[d/x] \models h$ for some $d \in \semantic{\kappa}$
\end{itemize}
