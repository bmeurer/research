...
%\begin{proof} \
%  \begin{enumerate}
%    \item It is sufficient to prove $\pi\,(\reachn{i}{L}{s}) = \reachn{i}{\pi(L)}{\pi^\ssto\,s}$ for every $i\in\N$:
%          \begin{itemize}
%            \item $\reachn{0}{\pi(L)}{\pi^\ssto\,s} = \pi(L) = \pi\,(\reachn{0}{L}{s})$
%
%            \item $\reachn{i+1}{\pi(L)}{\pi^\ssto\,s} \\
%                  = \reachn{i}{\pi(L)}{\pi^\ssto\,s}
%                    \cup \bigcup_{l\in\reachn{i}{\pi(L)}{\pi^\ssto\,s}} \fl{s(l)} \\
%                  = \pi\,(\reachn{i}{L}{s})
%                    \cup \bigcup_{l\in\reachn{i}{L}{s}} \fl{\pi^\sexp(s(l)} \\
%                  = \pi\,(\reachn{i+1}{L}{s})$
%          \end{itemize}
%
%    \item We prove only one direction, the reverse follows easily.
%
%          $\reach{\pi(L)}{\pi^\ssto\,s} = \pi\,(\reach{L}{s}) = \pi\,(\reach{L}{s'}) = \reach{\pi(L)}{\pi^\ssto\,s'}$
%
%          $l \in \reach{\pi(L)}{\pi^\ssto\,s} \\
%            \,\Rightarrow\,
%            \pi^{-1}(l) \in \reach{L}{s} \\
%            \,\Rightarrow\,
%            s(\pi^{-1}(l)) = s'(\pi^{-1}(l)) \\
%            \,\Rightarrow\,
%            (\pi^\ssto s)(l) = (\pi^\ssto s')(l)$
%
%    \item Again only one direction.
%
%          Let $s,s'\in\Store_L$ with $s \equiv_L s'$. Then $(\pi^\ssto\,s) \equiv_{\pi(L)} (\pi^\ssto\,s')$
%          where $(\pi^\ssto\,s),(\pi^\ssto\,s')\in\Store_{\pi(L)}$, hence $\phi\,(\pi^\ssto\,s)=\phi\,(\pi^\ssto\,s')$
%          and thereby $(\phi \circ \pi^\ssto) \in \DEF_L$.
%  \end{enumerate}
%\end{proof}
...
%  $D^\theta = \bigcup_{L \in W} D^\theta_L$ and $\I^\theta(\pi) : D^\theta \to D^\theta$ for every $\pi \in \Perm$ by
%  \begin{itemize}
%    \item $D^\tau_L = \{v\in\CVal^\tau\,|\,\supp{v} = L\}$ \\
%          $\I^\tau(\pi): D^\tau \to D^\tau, v \mapsto \pi^\sexp\,v$
%
%    \item $D^\tassn_L = \{ \phi\in\DEF_L\,|\,\forall \pi\in\Fix{L}.\,\phi = \phi \circ (\pi^\ssto)^{-1}\}$ \\
%          $\I^\tassn(\pi): D^\tassn \to D^\tassn, \phi \mapsto \phi \circ (\pi^\ssto)^{-1}$
%
%    \item $D^{\ttarrow{\theta_1}{\theta_2}}_L = \{ \psi: D^{\theta_1} \to D^{\theta_2}\,|\,
%              \forall L'\in W.\,\psi\,(D^{\theta_1}_{L'}) \subseteq D^{\theta_2}_{L \cup L'} \\
%              \hspace*{4.5cm} \wedge \forall \pi\in\Fix{L}.\,\psi\circ\pi^{\theta_1}=\pi^{\theta_2}\circ\psi\}$\\
%          $\I^{\ttarrow{\theta_1}{\theta_2}}(\pi):D^{\ttarrow{\theta_1}{\theta_2}}\to D^{\ttarrow{\theta_1}{\theta_2}},
%                              \psi \mapsto \I^{\theta_2}(\pi) \circ \psi \circ (\I^{\theta_1}(\pi))^{-1}$
%  \end{itemize}
%\end{definition}
%
