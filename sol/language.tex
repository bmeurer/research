...
%                    \cup \bigcup_{l\in\reachn{i}{\mu(L)}{\mu^\ssto\,s}} \fl{s(l)} \\
%                  = \mu\,(\reachn{i}{L}{s})
%                    \cup \bigcup_{l\in\reachn{i}{L}{s}} \fl{\mu^\sexp(s(l)} \\
%                  = \mu\,(\reachn{i+1}{L}{s})$
%          \end{itemize}
%
...
%to assert that expressions and values do not contain locations, i.e. $\fl{e} = \fl{v} = \emptyset$
...
%  we have
%  \begin{enumerate}
%    \item $(\semantic{\tj{t}{\sigma}}\,\eta) \in \semantic{\sigma}_{\supp{\eta|_{\fv{t}}}}$
%    \item $\semantic{\tj{t}{\sigma}}\,(\mu^\senv\,\eta) = \hat{\mu}^\sigma\,(\semantic{\tj{t}{\sigma}}\,\hat{\eta})$
%          whenever $(\mu^\senv\,\eta) =_{\fv{t}} (\hat{\mu}^\senv\,\hat{\eta})$
%  \end{enumerate}
%\end{lemma}
%
%\begin{proof}
...
%  Let $L = \supp{\eta|_{\fv{\tabstr{x}{t}}}}
%         = \supp{\eta|_{\fv{t} \setminus \{x\}}}$. In order to prove
...
%  \[\begin{array}{rcl}
%    \psi\,d = (\semantic{\tj{t}{\sigma'}}\,(\eta[d/x]))
%    &\in& \semantic{\sigma'}_{\supp{\eta[d/x]|_{\fv{t}}}}
%  \end{array}\]
%  and we conclude
%  \[\begin{array}{rcl}
%    \supp{(\eta[d/x])|_{\fv{t}}}
%    &=& \supp{\eta|_{\fv{t}\setminus\{x\}},d} \\
%    &=& L \cup \supp{d}.
%  \end{array}\]
...
%  \end{array}\]
%  whenever $(\mu^\senv\,\eta) =_{\fv{t}\setminus\{x\}} (\hat{\mu}^\senv\,\hat{\eta})$
...
%               $\eta =_{\fv{t}\setminus\{x\}} \hat{\eta}$, and
...
%          $(\mu^\senv\,\eta) =_{\fv{t}} (\hat{\mu}^\senv\,\hat{\eta})$, then
%          \begin{enumerate}
%            \item $(\semantic{\tj{p}{\pi}}\,\eta) \in \semantic{\pi}_{\supp{\eta|_{\fv{p}}}}$
...
%          $(\mu^\senv\,\eta) =_{\fv{h}} (\hat{\mu}^\senv\,\hat{\eta})$ we have
...
%          let $L = \supp{\eta|_{\fv{p}}}$. Then $\phi = \Neg\,(\semantic{\tj{p}{\tassn}}\,\eta)$ and
...
%          for all $\sigma\in\SType$, $t \in\Term^\sigma$ with $\fv{t} \subseteq X$.
%    \item $\semantic{\tj{p}{\pi}}\,\eta = \semantic{\tj{p}{\pi}}\,\hat{\eta}$
%          for all $\pi\in\PType$, $p \in \Assn^\pi$ with $\fv{p} \subseteq X$.
%    \item For every $h \in \Formula$
%          with $\fv{h} \subseteq X$ we have $\eta \models h$ iff $\hat{\eta} \models h$.
%  \end{enumerate}
%\end{theorem}
%
%\begin{proof}
