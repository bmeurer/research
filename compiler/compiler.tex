...
\begin{definition}[Ein- und Ausgabe]
  ...
  f"ur $g$. Dann sind f"ur alle $a \in A$ und $i \ge 0$ die Mengen $\inp_{g,\gamma}(a,i) \subseteq \N$ der
  \emph{Eingaben von $(a,i)$} und $\out_{g,\gamma}(a,i) \subseteq \N$ der \emph{Ausgaben von $(a,i)$} wie folgt
  induktiv definiert:
  \[\begin{array}{lcl}
    \inp_{g,\gamma}(a,0)
    &=& \emptyset \\
    ...
    &=& \out_{g,\gamma}(a,i) \\
    \out_{g,\gamma}(a,i)
    &=& \{j\in(\inp_{g,\gamma}(a,i)\cup\{i+1\}) \mid \gamma(a,j) > i \} \\
  \end{array}\]
\end{definition}
