...
\begin{definition}[Syntax der Categorical Abstract Machine]
  Vorgegeben sei eine (unendliche) Menge $\Loc$ von {\em Programmaddressen} $\ell$. Die Mengen $\Prog$ aller
...
Wir gehen im folgenden davon aus, dass jede Instruktion $I$ eine eindeutige Programmaddresse $\ell_i$ besitzt und
diese in geeigneter Weise vergeben werden, so dass die syntaktisch folgende Programmaddress durch $\ell_{i+1}$
...
Wir schreiben $P[\ell]$ f\"ur die Instruktion $I$ an der Programmaddresse $\ell\in\locns{P}$ im Programm $P$.
Weiterhin schreiben wir $\ell\langle I\rangle$ f\"ur die Programmaddresse $\ell\in\dom{P}$ mit $P[\ell]=I$.

\begin{definition}
  Die Menge $\dom{P} \subseteq \Loc$ aller Programmaddressen von $P$ ist wie folgt induktiv definiert:
  \[\begin{array}{rcl}
    \dom{\ell:I} &=& \{\ell\} \\
    \dom{P_1;P_2} &=& \dom{P_1} \cup \dom{P_2}
  \end{array}\]
  Die Menge $\locns{P} \subseteq \Loc$ aller in $P$ vorkommenden Sprungziele (implizit oder explizit) ist wie
  folgt induktiv definiert:
  \[\begin{array}{rcl}
    \locns{\ell_i:\textsc{Fst}} &=& \{\ell_{i+1}\} \\
    \locns{\ell_i:\textsc{Snd}} &=& \{\ell_{i+1}\} \\
    \locns{\ell_i:\textsc{Push}} &=& \{\ell_{i+1}\} \\
    \locns{\ell_i:\textsc{Swap}} &=& \{\ell_{i+1}\} \\
    \locns{\ell_i:\textsc{Quote}\,c} &=& \{\ell_{i+1}\} \\
    \locns{\ell_i:\textsc{Prim}\,\op} &=& \{\ell_{i+1}\} \\
    \locns{\ell_i:\textsc{Closure}\,\ell} &=& \{\ell_{i+1},\ell\} \\
    \locns{\ell_i:\textsc{App}} &=& \{\ell_{i+1}\} \\
    \locns{\ell_i:\textsc{Return}} &=& \emptyset \\
    \locns{\ell_i:\textsc{GotoFalse}\,\ell} &=& \{\ell_{i+1},\ell\} \\
    \locns{\ell_i:\textsc{Goto}\,\ell} &=& \{\ell\} \\
    \locns{\ell_i:\textsc{Skip}} &=& \{\ell_{i+1}\} \\
    \locns{\ell_i:\textsc{Stop}} &=& \emptyset \\
...
\begin{figure}
  ...
  \begin{center}
    \begin{tabular}{|rcl|l|}
      ...
      \hline
      $((R_1,R_2),S,\ell_i\langle\textsc{Fst}\rangle)$ & $\to$ & $(R_1,S,\ell_{i+1})$ & Erste Projektion \\
      $((R_1,R_2),S,\ell_i\langle\textsc{Snd}\rangle)$ & $\to$ & $(R_2,S,\ell_{i+1})$ & Zweite Projektion \\
      \hline
      $(R,S,\ell_i\langle\textsc{Push}\rangle)$ & $\to$ & $(R,R;S,\ell_{i+1})$ & Registerinhalt auf Stack \\
      $(R_1,R_2;S,\ell_i\langle\textsc{Swap}\rangle)$ & $\to$ & $(R_2,R_1;S,\ell_{i+1})$ & Registerinhalt und oberstes \\
      &&& Stackelement vertauschen \\
      \hline
      $(R,S,\ell_i\langle\textsc{Quote}\,c\rangle)$ & $\to$ & $(c,S,\ell_{i+1})$ & $c$ in Register laden \\
      $(z_2,z_1;S,\ell_i\langle\textsc{Prim}\,\op\rangle)$ & $\to$ & $(\op^I(z_1,z_2),S,\ell_{i+1})$ & Operation $\op$ ausf\"uhren \\
      $(R,S,\ell_i\langle\textsc{Closure}\,\ell\rangle)$ & $\to$ & $([R:\ell],S,\ell_{i+1})$ & Abschluss bilden \\
      \hline
      $([R_1:\ell],R_2;S,\ell_i\langle\textsc{App}\rangle)$ & $\to$ & $((R_1,R_2),\ell_{i+1};S,\ell)$ & Funktionsaufruf \\
      $([R:\ell],S,\ell_i\langle\textsc{Eval}\rangle)$ & $\to$ & $(R,\ell_{i+1};S,\ell)$ & Verz\"ogerte Auswertung \\
      &&& einer Closure \\
      $(R,\ell;S,\ell_i\langle\textsc{Return}\rangle)$ & $\to$ & $(R,S,\ell)$ & R\"ucksprung aus Closure \\
      $(\true,R;S,\ell_i\langle\textsc{GotoFalse}\,\ell\rangle)$ & $\to$ & $(R,S,\ell_{i+1})$ & Sprung zu $\ell$ wenn Register \\
      $(\false,R;S,\ell_i\langle\textsc{GotoFalse}\,\ell\rangle)$ & $\to$ & $(R,S,\ell)$ & den Wert $\false$ enth\"alt \\
      $(R,S,\ell_i\langle\textsc{Goto}\,\ell\rangle)$ & $\to$ & $(R,S,\ell)$ & Unbedingter Sprung zu $\ell$ \\
      $(R,S,\ell_i\langle\textsc{Skip}\rangle)$ & $\to$ & $(R,S,\ell_{i+1})$ & No-op \\
      \hline
    \end{tabular}
  \end{center}}
...
\begin{definition}[Berechnung]
  ...
  \begin{enumerate}
    \item {\em terminierend}, wenn sie endlich ist und der letzte Zustand die Form
      $(R_e,S_e,\ell_e\langle\textsc{Stop}\rangle)$ hat, oder
...
\[\begin{array}{rcl}
  ...
  \\
  \mathcal{C}\sem{e} &=& \textsc{Goto}\,\ell';\,\ell:\textsc{Skip};\,
                         \mathcal{V}\sem{e};\,\textsc{Return}; \\
                      && \ell':\textsc{Closure}\,\ell \\
  \\
  ...
  \mathcal{V}\sem{\app{e_1}{e_2}} &=& \textsc{Push};\,\mathcal{V}\sem{e_1};\,\textsc{Swap};\,
                                      \mathcal{C}\sem{e_2};\,\textsc{Swap};\,\textsc{App} \\
  \mathcal{V}\sem{e_1\,\op\,e_2} &=& \textsc{Push};\,\mathcal{V}\sem{e_1};\,\textsc{Swap};\,
                                     \mathcal{V}\sem{e_2};\,\textsc{Prim}\,\op \\
  \mathcal{V}\sem{\bifte{e_0}{e_1}{e_2}} &=& \textsc{Push};\,\mathcal{V}\sem{e_0};\,\textsc{GotoFalse}\,\ell;\,
                                             \mathcal{V}\sem{e_1}; \\
                                         &&  \textsc{Goto}\,\ell';\,\ell:\textsc{Skip};\,
                                             \mathcal{V}\sem{e_2};\,\ell':\textsc{Skip}
\end{array}\]
Wir nehmen an, dass die genaue Vergabe der Programmaddressen automatisch und korrekt geschieht. Dies ist
im Wesentlichen trivial und dementsprechend uninteressant.
...
\begin{quote}
  ...
  $(R_0,S_0,\ell_0) \xrightarrow* (R,S,\ell\langle\textsc{Stop}\rangle)$, so dass $R$ zu $(v\,\eta)$ {\em passt}. Und
